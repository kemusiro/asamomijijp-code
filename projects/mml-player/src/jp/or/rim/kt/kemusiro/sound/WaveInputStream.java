...
public class WaveInputStream extends InputStream {
	...
	MusicEvent e = (MusicEvent)events.getFirst();
	...
	for (int p = 0; p < getSampleCountPerTick(); p++) {
		...
		if (notes[ch] != null) {
		    value +=
			notes[ch].getVelocity() *
			insts[ch].getValue(notes[ch].getNumber());
		}
	    }
	...
	for (int t = 0; t < score.getTickPerBeat(); t++) {
	    ...
	    if (((MusicEvent)events.get(0)).getTick() == currentTick) {
		processEvent();
	    }
