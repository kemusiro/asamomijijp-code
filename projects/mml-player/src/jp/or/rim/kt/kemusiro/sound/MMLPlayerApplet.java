...
public class MMLPlayerApplet extends JApplet {
    ...
    public void init() {
	player =
	    new MMLPlayer(new LineListener() {
		    public void update(LineEvent e) {
			if (e.getType() == LineEvent.Type.STOP) {
			    printer.send("please input MML strings!");
			    }
			}
		    });
	...
	currentAlgorithm = 4;
	for (int i = 0; i < 8; i++) {
	    algorithmButton[i] = new JRadioButton(Integer.toString(i),
						  i == currentAlgorithm);
    ...
    class FMButtonListener implements ActionListener {
